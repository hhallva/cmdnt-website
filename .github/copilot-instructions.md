...
- `backend` hosts a .NET 8 Web API (project `API`) plus a shared core library (`Core`); controllers live under `backend/API/Controllers`, while entity definitions/mappers/DTOs sit under `backend/Core`.
...
- All API routes are versioned under `/api/v1/...` and return either DTOs (via `Core/DTOs/*Mapper.cs`) or `ApiErrorDto` for failures; frontend types in `frontend/src/types` mirror those DTOs 1:1.
...
- Keep DTO mapping logic inside `Core/DTOs/*Mapper.cs` (e.g., `StudentMapper.ToDto`) and load navigation properties before calling `ToDto` because mappers assume related entities (`Group`, `Rooms`) are already included.
- Central error semantics live in `Core/DTOs/ApiErrorDto`; reuse it for every non-200 result to stay consistent with the global exception handler defined at the bottom of `Program.cs`.
- When touching persistence, update `Core/Models`, `AppDbContext`, and the EF migrations under `backend/Core/Migrations`; run `dotnet ef database update --project backend/Core --startup-project backend/API` after adding migrations.
...
## Developer Workflow
- Backend: `dotnet restore backend/backend.sln` → `dotnet watch run --project backend/API`; provide `DB_PASSWORD`, `JWT__Key`, and optionally `ASPNETCORE_ENVIRONMENT` via user secrets or environment variables. Swagger is available at `http://localhost:5000/swagger`.
...
- Builds: `npm run build --prefix frontend` emits static assets under `frontend/dist`; `dotnet publish backend/API -c Release` produces the API artifact.
...
- Shared DTOs: `backend/Core/DTOs/**` ↔ `frontend/src/types/**`.
